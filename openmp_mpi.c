...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0)
    {
        ...
        // char customState[] = "RRGRRWRRGYOOBOOYOOWWWYYYWWWBGBBGBBGBYYRBBRYYRGGOWWOGGO";
        // cube = init_rubiks_cube(3, colours, customState);
    ...
    #pragma omp parallel num_threads((2 * (2 * cube->n + 6) - rank) / size)
    for(int i=rank; i<2*(2*cube->n+6); i+=size){
        ...
        if (PRINT_REQ)
            printf("%d]%d-%d-%d\n", rank, type, value, dir);
        RubiksCube *cur_cube = init_rubiks_cube(cube->n, cube->colours, stringify(cube));
        char temp[MAX_MOVES];
        switch(type){
            case 0:
                horizontal_twist(cur_cube, value, dir);
                temp[0] = 'h';
                break;
            case 1:
                vertical_twist(cur_cube, value, dir);
                temp[0] = 'v';
                break;
            case 2:
                side_twist(cur_cube, value, dir);
                temp[0] = 's';
                break;
            }
            temp[1] = '0' + value;
            temp[2] = '0' + dir;
            solve(cur_cube, 1, temp, 3, path);
            if (PRINT_REQ)
                printf("%d]%s\n", rank, path);
            switch (type)
            {
            case 0:
                horizontal_twist(cur_cube, value, 1 - dir);
                break;
            case 1:
                vertical_twist(cur_cube, value, 1 - dir);
                break;
            case 2:
                side_twist(cur_cube, value, 1 - dir);
                break;
        }
    }
